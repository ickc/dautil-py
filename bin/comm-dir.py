...
def main(args):
    ...
    if args.verbose:
        print(
            "There are {} input files and {} output files.".format(
                len(in_files), len(out_files)
            )
        )
        if len(diff) > 0:
            print(
                "There are {} files that are in input directory but not in output directory.".format(
                    len(diff)
                )
            )
            print("And here's it:")
...
def cli():
    parser = argparse.ArgumentParser(
        description="Compare 2 directories and print differences."
    )

    parser.add_argument("indir", help="Input directory.")
    parser.add_argument("-o", "--outdir", required=True, help="Output directory.")
    parser.add_argument(
        "-I", "--in-glob", default="*", help="Input glob pattern. Default: *"
    )
    parser.add_argument(
        "-O", "--out-glob", default="*", help="Output glob pattern. Default: *"
    )
    parser.add_argument(
        "-b",
        "--basename",
    ...
    parser.add_argument(
        "-R", "--recursive", action="store_true", help="If specified, glob recursively."
    )
