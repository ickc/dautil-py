...
def main(args):
    ...
    pkl_in_paths = chain(*(_glob(glob_i) for glob_i in args.input))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Delete pickle input if it is invalid."
    )

    parser.add_argument("input", nargs="+", help="glob pattern of pickle files.")
    parser.add_argument(
        "-R",
        "--recursive",
        action="store_true",
    ...
    parser.add_argument(
        "-n", "--dry-run", action="store_true", help="Dry run, do not delete."
    )
    parser.add_argument(
        "-V",
        "--verbose",
