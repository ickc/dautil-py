...
def main(args):
    ...
    in_paths = chain(
        *(_glob(os.path.join(args.basedir, glob_i)) for glob_i in args.glob)
    )
...
def cli():
    parser = argparse.ArgumentParser(
        description="Convert pickle to pickle in a certain protocol."
    )

    parser.add_argument("basedir", help="Base directory of input pickle files.")
    parser.add_argument(
        "-o", "--output", required=True, help="Base directory of output pickle files."
    )
    parser.add_argument(
        "--glob", required=True, nargs="+", help="Glob pattern from BASEDIR."
    )
    parser.add_argument(
        "-R",
        "--recursive",
    ...
    parser.add_argument(
        "--protocol", type=int, default=2, help="Output pickle procotol. Default: 2."
    )
    ...
    parser.add_argument("--mpi", action="store_true", help="If specified, use MPI.")
    parser.add_argument(
        "-p",
        "--processes",
        type=int,
